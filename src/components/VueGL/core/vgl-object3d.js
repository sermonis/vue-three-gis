...
export default {
  ...
  created() { this.vglWorld.update(); },
  beforeUpdate() { this.vglWorld.update(); },
  beforeDestroy() {
    const { vglWorld, inst, name } = this;
    if (inst.parent) inst.parent.remove(inst);
    if (vglWorld.object3ds[name] === inst) delete vglWorld.object3ds[name];
    vglWorld.update();
  },
  watch: {
    inst: {
      handler(inst, oldInst) {
        ...
        if (this.name !== undefined) this.vglWorld.object3ds[this.name] = inst;
      },
      immediate: true,
    },
    ...
    name(name, oldName) {
      const { vglWorld: { object3ds }, inst } = this;
