...
export default {
  ...
  computed: {
    inst: () => new THREE.Camera(),
  },
  watch: {
    inst: {
      handler(inst) {
        this.vglWorld.cameras[this.name] = inst;
        if (this.orbitPosition || this.orbitTarget) {
          ...
          inst.lookAt(target || new THREE.Vector3());
        }
      },
      immediate: true,
    },
    name(name, oldName) {
      const { vglWorld: { cameras }, inst } = this;
    ...
    orbitPosition(orbitPosition) {
      ...
      if (this.orbitTarget) {
        ...
        this.inst.lookAt(new THREE.Vector3());
      }
    },
  },
  beforeDestroy() {
